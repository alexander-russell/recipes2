...
new_unit = Unit.objects.filter(singular="gram").first()
if not new_unit:
    raise ValueError("No unit found with singular='cup'")

# Find and update all items that currently use "cups"
old_unit = Unit.objects.filter(singular="hey").first()
if not old_unit:
    raise ValueError("No unit found with singular='cups'")
